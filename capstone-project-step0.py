...
for data_str in data_list:
    ...
    for stud_name in stud_list:
        ...
        # fraction of short attemps (with time <= 10 sec)
        if (num_attempts > 0):
            frac_10s_atts = stud_info_df[(stud_info_df['Student Response Type'] == 'ATTEMPT') & \
                                   (stud_info_df['Duration (sec)'].replace({'.': 0}).astype(float) <= 10.0)].shape[0] / num_attempts
        else:
            frac_10s_atts = 0
        ...
        if (num_hints > 0):
            frac_5s_hints = stud_info_df[(stud_info_df['Student Response Type'] == 'HINT_REQUEST') & \
                                   (stud_info_df['Duration (sec)'].replace({'.': 0}).astype(float) <= 5.0)].shape[0] / num_hints
        else:
            frac_5s_hints = 0
        ...
        stud_data.loc[stud_name, 'frac_10s_atts'] = frac_10s_atts
        stud_data.loc[stud_name, 'frac_5s_hints'] = frac_5s_hints
